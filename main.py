...
def simulation(beeName):
    ...
    movementSeries = []
    gatherSeries = []
    tokenActivations = {token: [] for token in tlist}
    print("script has started")
    start = timer()
    while currentTime < ELAPSETIME:
        if currentTime % plotInterval == 0:
            timeSeries.append(currentTime)
            movementSeries.append(timers['movement'])
            gatherSeries.append(timers['gather'])
            for token in tlist:
                tokenActivations[token].append(timers[token]['TGC'])
        # decrease cooldown
        for token in timers:
    ...
    plt.legend()
    plt.subplot(2,1,1)
    for token, data in tokenActivations.items():
        plt.plot(timeSeries, data, label=f"token: {token}")
    plt.xlabel("time - ms")
